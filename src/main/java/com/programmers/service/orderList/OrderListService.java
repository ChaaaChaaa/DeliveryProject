...
public class OrderListService {
	...
	public OrderList save(OrderRequest orderRequest) {
		...
		Delivery delivery = deliveryRepository.findById(orderRequest.getDeliveryId())
	...
	public OrderList save(OrderRequestDto orderRequestDto) {
		// order save
		User user = userRepository.findByUserId(orderRequestDto.getUser().getUserId())
			.orElseThrow(UserNotFoundException::new);
		Delivery delivery = deliveryRepository.findById(orderRequestDto.getDelivery().getDeliveryId())
			.orElseThrow(DeliveryNotFoundException::new);
		...
		// orderItemList save
		orderItemRepository.saveAll(orderRequestDto.getOrderItems(savedOrderList.getOrderListId()));
		return savedOrderList;
	}

	public OrderResponseDto findById(Long orderListId) {
		return OrderResponseDto.of(orderListRepository.findById(orderListId)
			.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "해당 id의 주문이 존재하지 않습니다.")));
	}
	...
	public Delivery findDeliveryByOrderListId(Long orderListId) {
		OrderList orderList = orderListRepository.findById(orderListId).orElseThrow(UserNotFoundException::new);
		return deliveryRepository.findById(orderList.getDelivery().getDeliveryId())
			.orElseThrow(DeliveryNotFoundException::new);
	}
